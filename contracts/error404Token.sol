...
contract error404Token is BEP20('error404 Token', '404') {
    ...
    external
    {
        ...
        require(signatory != address(0), "error404::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "error404::delegateBySig: invalid nonce");
        require(now <= expiry, "error404::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "error404::getPriorVotes: not yet determined");
    ...
    internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying error404s (not scaled);
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "error404::_writeCheckpoint: block number exceeds 32 bits");
